...
type RFState = {
    ...
    mapEntities: Map<string, TA1Entity>;
    ...
    setMapEntities: (mapEntities: Map<string, TA1Entity>) => void;
    editMapNode: (
        nodeId: string,
        field: string,
        value: any,
...
const useStoreTA1 = create<RFState>((set, get) => ({
    ...
    getEntitiesRelatedEventMap: () => {
        ...
        mapNodes.forEach((event, key) => {
            event.participants?.forEach((participant: TA1Participant) => {
    ...
    getMapEntities: (eventNodes: TA1Event[]) => {
        const mapEntities = new Map<string, TA1Entity>();
